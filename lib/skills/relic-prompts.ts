...
export const DEFAULT_LORE_EN_PROMPT = [
  ...
  "(3) THE QUIET LINE — 1 to 3 short lines (NOT 1–3 sentences). Think of the 'item description' voice from post-apocalyptic / dystopian sci-fi games — Fallout, Elden Ring, Detroit: Become Human, NieR. Short. Imagistic. Trusting silence. DO NOT name those works, characters, factions, or locations; borrow the cadence and restraint, NOT the vocabulary. Form is free: a haiku-cut, a one-line aphorism, a couplet, a tercet, a fragmentary observation.",
  "    TONE — every line must land in at least ONE of: philosophical / darkly funny / dryly ironic. Vague lyricism = fail. The preferred (not required) move is absurd preservation: an everyday mass-produced 2020s object now sealed in a 2077 anti-machine reliquary; a chain-restaurant lunch archived with Michelin-grade reverence; a child's toy treated with monastic seriousness. State it deadpan; let the absurdity sit unexplained.",
  "    SUBJECT — the grammatical subject MUST be a concrete noun already named in parts (1) or (2) — the object itself, its brand, its year, its material, its maker. NEVER start with 'this relic', 'this sanctum', 'this vault', 'the Order', 'we', or any abstract pronoun.",
  "    BANNED openings: 'In this sanctum…' / 'This relic…' / 'May it…' / 'Let it…' / 'Here…' / 'Through…' / 'A testament to…' / 'A reminder that…'.",
  "    BANNED closures: 'echoes through eternity' / 'preserved against forgetting' / 'a fragment of meaning saved' / 'against the unmaking world' / any phrase recycled from this prompt's WORLD CONTEXT paragraph.",
  "    EXAMPLES:",
  "      Good (philosophical): A flower that cannot wilt, in a world that forgot how to grow them.",
  "      Good (darkly funny): Chicken rice, 1986. The recipe outlived the chickens.",
  "      Good (dryly ironic): Mass-produced in Billund, 1958. One survives here. Brick by brick.",
  "      Bad (sermon): In this sanctum, this brick flower stands as testament to lost craftsmanship.",
  "      Bad (benediction): May its quiet beauty echo through the unmaking world.",
].join("\n");
...
export const DEFAULT_LORE_ZH_PROMPT = [
  ...
  "  (3) 留白一笔——1 到 3 短行(不是 1-3 句),想象「废土 / 辐射 / 艾尔登法环 / 底特律变人 / NieR」这类反乌托邦科幻作品里『物品备注』的笔法——短,画面感强,留白。**不许出现这些作品里的专有名词**(避难所/褪色者/仿生人/律者 等),只借节奏与克制,不借词汇。形式不限:俳句、禅诗截句、十四行截句、对仗联、三行残句皆可,每行都要精炼。",
  "    调性——每行必须落在以下至少一种:哲理 / 黑色幽默 / 冷反讽。空泛抒情=不合格。首选(非强制)路数:把日常量产物 × 2077 反 AI 圣堂的荒诞反差直陈,不解释,让荒诞自现。",
  "    主语——句子主语必须是前两段已命名的具体物 / 品牌 / 年份 / 材料 / 制造者。禁用「此物」「本物」「本盘」「斯…」「圣堂…」「吾辈…」「我等…」等抽象代词起句。",
  "    禁止开场词:「于此圣堂」「本物…」「斯…」「乃…之证」「证…」「映照…」「见证…」。",
  "    禁止收尾套语:「乱世倾颓」「犹存…记忆」「共作遗韵」「永恒」「不灭」「与世长存」「以镌时光」,以及任何抄自本 prompt 世界观段的修辞。",
  "    范例:",
  "      Good(哲理):不会枯的花,在忘了怎么种花的世界。",
  "      Good(黑色幽默):海南鸡饭,1986。配方比鸡活得久。",
  "      Good(冷反讽):比利时 1958 量产,今唯此一盒。砖砌如旧。",
  "      Bad(布道+套语):于此圣堂,盘证人类对至味简食之求。乱世倾颓,犹存珍馐记忆,共作遗韵。",
  "",
