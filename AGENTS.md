...
- `Skill` —— 可装备的能力定义（kind = PASSIVE/ACTIVE/ULTIMATE，level 1-6，icon 走 Material Symbols 名）。CRUD 在 `/agent-control?tab=skills`。
...
赛博朋克 2077 风格的装备面板，承载站内所有 AI workflow（machine，黄色）和 AI agent（agent，青绿）。"ai-clergy" 是已废弃的旧名，**不要再创建 `/ai-clergy` 路由**——所有迭代统一在 `/agent-control` 下。
...
- machine（脊柱）—— 三对左右对称排列；背景 `/public/images/agent-control/spine.jpg`，缺图自动 fallback `spine.svg`。
