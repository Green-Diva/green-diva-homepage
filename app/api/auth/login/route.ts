...
import { deriveTokenLookup, verifyToken } from "@/lib/userToken";
...
export async function POST(req: NextRequest) {
  ...
  const lookup = deriveTokenLookup(submittedToken);
  const user = await prisma.user.findUnique({ where: { tokenLookup: lookup } });
  const tokenOk = user ? await verifyToken(submittedToken, user.tokenHash) : false;
  if (!user || !tokenOk) {
    recordFailure(key);
    await new Promise((r) => setTimeout(r, FAIL_DELAY_MS));
