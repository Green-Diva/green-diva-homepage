...
import type { ClericSkillKind } from "@/lib/clericTypes";
...
const KIND_OPTIONS: ClericSkillKind[] = ["PASSIVE", "ACTIVE", "ULTIMATE"];
...
function blank(initial?: SkillRow | null) {
  return {
    ...
    kind: (initial?.kind ?? "PASSIVE") as ClericSkillKind,
...
export default function SkillEditor({ mode, initial, onClose, onSaved }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!r.ok) {
      ...
      setErr(typeof j.error === "string" ? j.error : t.aiClergy.skillSaveFailed);
      return;
    }
    router.refresh();
    onSaved();
  ...
  async function onDelete() {
    if (!initial) return;
    if (!confirm(format(t.aiClergy.skillDeleteConfirm, { name: initial.nameZh || initial.nameEn }))) return;
    const r = await fetch(`/api/skills/${initial.id}`, { method: "DELETE" });
    if (!r.ok) {
      alert(t.aiClergy.skillDeleteFailed);
      return;
    }
    router.refresh();
    onSaved();
  ...
  return createPortal(
    <div
      role="dialog"
      aria-modal="true"
      aria-label={mode === "create" ? t.aiClergy.skillCreateNew : t.aiClergy.skillEdit}
        ...
        <div className="cyber-panel rounded-lg p-6 flex flex-col gap-5">
          <div className="flex items-center justify-between gap-4">
            <h2 className="font-label text-[11px] tracking-[0.3em] text-primary uppercase">
              {mode === "create" ? t.aiClergy.skillCreateNew : t.aiClergy.skillEdit}
            </h2>
            <button
              ref={closeBtnRef}
              type="button"
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={v.kind}
                  onChange={(e) => upd("kind", e.target.value as ClericSkillKind)}
                  className={inputCls}
                  required
                >
                  {KIND_OPTIONS.map((k) => (
