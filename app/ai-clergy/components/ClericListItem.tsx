...
const STATUS_STYLES: Record<ClericRow["status"], { dot: string; label: keyof ReturnType<typeof useT>["aiClergy"] }> = {
...
const MODE_ICON: Record<ClericRow["mode"], { icon: string; color: string; key: "modeMechanical" | "modeAutonomous" }> = {
...
export default function ClericListItem({
  cleric,
  active,
  onSelect,
}: {
  ...
  const t = useT();
  const status = STATUS_STYLES[cleric.status];
  const label = t.aiClergy[status.label] as string;
  const mode = MODE_ICON[cleric.mode ?? "MECHANICAL"];

  return (
    <button
      type="button"
      onClick={() => onSelect(cleric.id)}
      aria-pressed={active}
      className={[
    ...
    >
      ...
      <div className="relative shrink-0">
        ...
        <span
          aria-hidden
          className={`absolute -bottom-[1px] -right-[1px] w-2.5 h-2.5 rounded-full border border-background ${status.dot} ${cleric.status === "ONLINE" ? "mv-status-dot" : ""}`}
        />
      </div>
      <div className="min-w-0 flex-1">
        <div className={`font-label text-[11px] tracking-[0.2em] flex items-center gap-1.5 ${active ? "text-primary" : "text-on-surface"}`}>
          <span
            ...
            title={t.aiClergy[mode.key] as string}
          >
            {mode.icon}
          </span>
          {cleric.codename}
        ...
        <div className="font-label text-[10px] tracking-[0.18em] text-on-surface-variant truncate">
          {cleric.nameZh} · {cleric.nameEn}
        </div>
      </div>
      <span
        className={`font-label text-[9px] tracking-[0.25em] uppercase ${
          cleric.status === "ONLINE" ? "text-emerald-400" : cleric.status === "STANDBY" ? "text-amber-200" : "text-rose-300"
        }`}
      >
        {label}
      </span>
