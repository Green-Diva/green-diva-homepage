...
export default function SkillLibrary({ skills, equipsByClericId, activeClericId, isAdmin }: Props) {
  const t = useT();
  ...
  const activeEquips = activeClericId ? (equipsByClericId[activeClericId] ?? []) : [];
  const equippedIds = new Set(activeEquips.map((e) => e.skillId));

  async function toggleEquip(skill: SkillRow) {
    if (!activeClericId) return;
    setBusyEquip(skill.id);
    const isEquipped = equippedIds.has(skill.id);
    try {
      if (isEquipped) {
        await fetch(`/api/clerics/${activeClericId}/skills/${skill.id}`, { method: "DELETE" });
      } else {
        await fetch(`/api/clerics/${activeClericId}/skills`, {
          method: "POST",
    ...
    <div className="flex-1 overflow-y-auto p-5 md:p-8 flex flex-col gap-6">
      ...
      {skills.length === 0 ? (
        <div className="flex-1 flex items-center justify-center">
          <p className="text-on-surface-variant text-sm">{t.aiClergy.skillEmptyLibrary}</p>
        </div>
      ) : (
              ...
              <div key={lv}>
                <div className="flex items-center gap-3 mb-3">
                  <span className="font-label text-[10px] tracking-[0.3em] text-primary/60 uppercase">
                    {format(t.aiClergy.skillLevel, { n: lv })}
                  </span>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="font-label text-[8px] tracking-[0.12em] text-on-surface-variant/50 uppercase">
                            {format(t.aiClergy.skillCostAp, { n: skill.costAp })}
                          </span>
                          {isEquipped && (
                          ...
                          <div className="mt-auto pt-1">
                            ...
                            >
                              {busyEquip === skill.id
                                ? "…"
                                : isEquipped
                                  ? t.aiClergy.skillUnequip
                                  : t.aiClergy.skillEquip}
                            </button>
                          </div>
                        )}
                      </CyberPanel>
