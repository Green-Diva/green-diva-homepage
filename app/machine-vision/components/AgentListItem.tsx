...
const STATUS_STYLES: Record<AgentRow["status"], { dot: string; label: keyof ReturnType<typeof useT>["machineVision"] }> = {
  ONLINE: { dot: "bg-secondary", label: "statusOnline" },
...
export default function AgentListItem({
  ...
  const label = t.machineVision[status.label] as string;
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className={`font-label text-[11px] tracking-[0.2em] ${active ? "text-primary" : "text-on-surface"}`}>
          {agent.codename}
        </div>
      ...
      <span
        className={`font-label text-[9px] tracking-[0.25em] uppercase ${
          agent.status === "ONLINE" ? "text-secondary" : agent.status === "STANDBY" ? "text-amber-200" : "text-rose-300"
        }`}
      >
        {label}
      </span>
