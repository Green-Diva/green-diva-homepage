...
- `Agent` —— `/agent-control` 模块的 workflow / agent 实体，关键字段：`mode`（`MECHANICAL` 黄色 / `AUTONOMOUS` 青绿）、`avatarUrl` **NOT NULL**、`capabilities String[]`（Phase 0a：admin 维护的能力标签 array，SceneBindingEditor 按"requiredCapabilities ⊆ agent.capabilities"过滤候选 agent）、4 项派生 stats（`chaosLevel/costTier/activityLevel/stabilityLevel`，本期都 0% 占位）、`pipelineConfig` Json?（v1 线性 / v2 DAG 双格式，详见 Skill Handler 段）、`dispatcherConfig` Json?（agent 中央槽 orchestrator 调度）、`deployedAt`（草稿 vs 已部署）。详见下方"Agent Service + Forge Agents"段。
- `Skill` —— 可装备的能力定义（level 1-6，icon = Material Symbols 名）。CRUD 在 `/agent-control?tab=skills`。**两组类型字段并存**：① `kind` (PASSIVE/ACTIVE/ULTIMATE) 是**纯装饰**（仅 [`SkillLibrary.tsx`](app/agent-control/components/SkillLibrary.tsx) 用于徽章配色，零 runtime 语义）；② `handlerKind` (HTTP_API/LLM_PROMPT/MCP_SERVER/INTERNAL) 是 **runtime 路由字段**，决定 [`lib/skills/registry.ts`](lib/skills/registry.ts) 派给哪个 handler。同时存的字段还有 `handlerConfig` Json（每个 handler 自己定义 shape）+ `inputSchema/outputSchema` Json?（JSON Schema，invoke 时校验 IO + 给 Orchestrator 转 LLM tool 定义）。SkillEditor UI Phase 3 起改成"What this skill does" preset 选择，handlerKind 隐藏到 Advanced。详见 [docs/skill-handler-system.md](docs/skill-handler-system.md)。
- `SceneBinding` —— Phase 0a 落地。把 code-registered scene（`relic.enhance2d` 等）映射到 agent。字段：`sceneKey @unique` / `agentId` / `inputMap Json` (template `{{ctx.X}}` / `{{actor.X}}` 派 ctx → AgentJob.input) / `outputMap Json?` (callScene/runner 用，scope `{ output, runLog: { entries, byId }, ctx, actor }`) / `enabled Boolean` / `notes String?` / `rolloutPct Int @default(100)` (Phase 6.2 灰度) / `fallbackAgentId String?` (Phase 6.2 备用代理，无 FK relation 防 cascade)。`onDelete: Restrict` on agent FK — 删 agent 前必须先 unbind。详见"Agent Service + Forge Agents"段。
- `AgentJob` —— async 调用历史，与 `RelicProcessingJob` 同模式。`mode` 字段是调用时刻的 agent.mode 快照（防 retry 飘移），`runLog` Json 是每步执行轨迹（数组），`maxAttempts=3`，状态机 `PENDING → RUNNING → SUCCESS|FAILED`。Crash recovery 由 [`lib/server-init.ts::ensureServerInit()`](lib/server-init.ts) 统一处理（与 RelicProcessingJob 同入口，10 分钟阈值）。**Phase 7 加 4 列 telemetry**：`sceneKey` / `actorUserId` / `actorName` / `routedTo` (`"primary"|"fallback"`)，由 `dispatchScene`/`callScene` 自动写入；直接 `invokeAgent` 留 null。Activity tab 按这些字段过滤 + 展示。
...
app/
  ...
  admin/users/          — 管理员用户管理
  profile/              — 当前用户主页
  agent-control/      — Agent / Skill 装备界面（4 tab：agents 默认 / skills 库 CRUD / scenes 绑定 / activity 调用历史）
  api/
    ...
    agents/             — GET / POST，[id] PATCH/DELETE，[id]/skills POST/[skillId] PATCH-DELETE，
                          ...
                          [id]/export GET（Phase 4: 自包含 JSON envelope），
                          import POST（Phase 4: codename + skill 冲突解析）
    agent-jobs/         — GET（Phase 7: list + filterOptions, query: sceneKey/agentId/status/sinceMs/limit）
                          [jobId] GET（通用 polling，admin-only）
...
# GEMINI_API_KEY="..."        # LORE-FORGE 必填——loreEn (grounding+vision) + loreZh + metadata 三个 LLM_PROMPT skill；smartImagePicker vision filter
...
# SERPAPI_KEY="..."           # 可选——smartImagePicker 网图候选；不设自动降级到只用用户图
...
Draft phase (RelicDraft):
  ...
  GENERATE_METADATA (50%)   callScene("relic.draft-metadata", { workspaceSlug })
                              → SceneBinding 路由到 LORE-FORGE-001 (Phase 5+)
                              → outputMap 把 runLog node outputs 塑形为 { research, pick }
                              → pipeline step writeback 到 RelicDraft.generatedMetadata
...
## Agent Service + Forge Agents（Phase 0-8）
...
浏览器 → POST /api/relics/[id]/enhance-2d              ← endpoint 永不变
        ...
        Scene Registry (lib/scenes-init.ts → lib/relics/scenes.ts)：找定义 + 校验 contextSchema
              ↓
        SceneBinding 表 (DB)：admin 改 agentId / inputMap / outputMap / rolloutPct / fallbackAgentId
              ↓
        applyTemplate(inputMap, { ctx, actor }) → AgentJob.input
              ↓
        invokeAgent → backbone DAG → handler (HTTP_API / LLM_PROMPT / INTERNAL)
```

### Forge agents (Phase 5+ 落地)

替换原"单 SCRIBE-001 4-mode router"设计。每个 forge 专职一个能力域，DAG 由 LLM_PROMPT + HTTP_API + 极少 INTERNAL skills 组装。SCRIBE-001 已 Phase 6.1 retire（pipelineConfig 清空、equips 清空，row 保留作历史记录，admin 可手删）。

| Forge | 职责 | 槽位结构 |
|---|---|---|
| **LORE-FORGE-001** | 草稿元数据 + regen（绑 `relic.draft-metadata` + `relic.regen-metadata` scenes） | summary (INTERNAL) → loreEn (LLM_PROMPT Gemini grounding+vision) → loreZh (LLM_PROMPT Gemini text) → metadata-init (LLM_PROMPT Gemini json+vision) → pick (INTERNAL smart-image-pick) ; mode-router branches to metadata-regen for regen mode |
| **CUTOUT-FORGE-001** | 2D 抠图（绑 `relic.enhance2d` scene） | image-to-data-uri (INTERNAL) → fal-cutout-http (HTTP_API + download) → save-asset-enhanced (HTTP_API → /api/internal/save-asset，emit `_relicWriteback`) |
| **MESHY-FORGE-001** | 3D 生成（绑 `relic.create3d` scene） | image-to-data-uri (INTERNAL) → meshy-3d-http (HTTP_API + polling + download) → save-asset-relic (HTTP_API + emit `_relicWriteback`) |

**Skill 复用**：`image-to-data-uri` (INTERNAL) 同时被 CUTOUT + MESHY 两个 forge 装备 — Skill 是 mode-agnostic 共享资产的真实例证。`relic-files-summary` 同时被 SCRIBE history + LORE-FORGE 引用。
...
### Pipeline-step / endpoint 解耦（Phase 5）

老 SCRIBE 设计要求 [generateMetadata.ts](lib/relics/pipeline/steps/generateMetadata.ts) 按硬编码 node ID (`research`, `pick`) 从 runLog 抽 output — agent 重命名 node 就破。Phase 5 R1 改成读 `result.output`，由 SceneBinding.outputMap 用 `{{runLog.byId.<nodeId>.output.X}}` 模板把字段塑形。

`callScene` 给 outputMap scope 暴露 `{ output, runLog: { entries, byId }, ctx, actor }`。template engine path 支持 dash 让 `metadata-regen` 这种 node ID 能引用。新 LORE-FORGE binding 的 outputMap 把 metadata-init / loreEn / loreZh / pick 的 output 拼成 pipeline step 期望的 `{ research, pick }` shape。

### 异步 writeback hook（数据驱动）

[`runner.ts::maybeWriteRelicAsset`](lib/skills/runtime/runner.ts) 两条路径：
1. **数据驱动（首选）**：agent leaf output 含 `_relicWriteback: { id, fields }` → 按 **15 字段 allowlist**（`enhancedImagePath` / `modelPath` / `loreZh` 等）写 Relic 列。新增 mode 不动 runner 代码 — 由 SceneBinding outputMap 或 skill responseTransform 产出 _relicWriteback。
2. **Legacy 硬编码**：`mode === "2dEnhance" → enhancedImagePath` / `mode === "3dCreate" → modelPath`。Phase 6.1 起没有 agent 走这条路（forge agents 都用数据驱动），保留为 rollback 兜底。
...
### 灰度 / fallback (Phase 6.2)

SceneBinding 字段 `rolloutPct` (0-100) + `fallbackAgentId`。dispatch.ts 每次调用按 `Math.random() * 100 < rolloutPct` 投骰子；非 primary 分到 fallback。其中一边不可用时自动 rescue 到另一边。`routedTo: "primary" | "fallback"` 写进 AgentJob 列，Activity tab 可视化。

### Activity tab (Phase 7)

`AgentJob` 加 4 列：`sceneKey` / `actorUserId` / `actorName` / `routedTo`，由 `dispatchScene`/`callScene` 自动写入（直接 `invokeAgent` 留 null）。`/agent-control?tab=activity` 用 `GET /api/agent-jobs` 端点过滤 + 展示，配合 6.2 灰度看 A/B 效果。

### Loop primitive (Phase 8 R1)

backbone DAG v2 第三种 node type：`loop`。Body 是自包含 sub-DAG（own nodes + edges），每次 iteration 用前次 leaf output 作 input，遇到 exitWhen 匹配 / 达 maxIterations 退出。aggregate=`"last"`/`"concat-array"`。`MAX_LOOP_DEPTH = 2` 防递归爆栈。runtime 里 `runBackbone` 通过 `_internalEquips` / `_depth` / `_runLog` / `_stepIdPrefix` internal opts 递归调自身处理 body — 不抽 helper、不破坏现有 DAG。详见 [backbone.ts](lib/skills/runtime/backbone.ts)。
...
### Prompt hoist (Phase 5 R2)

`DEFAULT_LORE_EN_PROMPT` / `DEFAULT_LORE_ZH_PROMPT` / `DEFAULT_METADATA_PROMPT` 从 `geminiResearcher.ts`（已删）hoist 到 [`lib/skills/relic-prompts.ts`](lib/skills/relic-prompts.ts)（无 `server-only`，让 migrate scripts 也能 import）。LORE-FORGE 的 3 个 LLM_PROMPT skill 在 migrate-lore-forge 创建时 seed 这些 prompt 作为 systemPrompt 默认值；admin 在 SkillLibrary 改 `handlerConfig.systemPrompt` 即覆盖。
...
- **Tab key**：`agents` / `skills` / `scenes` / `activity`（[`AgentClient.tsx`](app/agent-control/AgentClient.tsx) `TabKey`）。`agents` 下显示 roster 与详情；`skills` 下做 SkillLibrary CRUD；`scenes` 下编辑 SceneBinding (Phase 0c)；`activity` 下看跨模块 AgentJob telemetry (Phase 7)。这里的 "agents" 指 tab 名，**不是** mode 名。Phase 4 起 roster footer 加 Export ↓ / Import ↑ 按钮（导入返回 deployedAt=null 的 agent，admin 测试后 deploy）。
...
**代码组织**：
- `lib/agent-service/` —— scene 分发层（Phase 0a）：`registry.ts` / `template.ts` / `dispatch.ts` / `serialize.ts` / `types.ts` / `index.ts` (barrel)。模块在 `lib/<module>/scenes.ts` 调 `registerScene(...)`，`lib/scenes-init.ts` 中央 import 触发副作用。
- `lib/skills/handlers/` —— handler 实现：
  - `httpApi.ts` —— 通用 REST 调用器。Phase 2.2 加 `polling` (intervalMs/timeoutMs/successWhen/failureWhen) + `download` (urlPath → base64 + content-type) + `responseTransform` (template scope = `{ input, response }`)。authScheme: `"Bearer"|"ApiKey"|"Key"|"Basic"|"Header"`（"Key" 是 fal.ai 风格 `Authorization: Key <key>`）
  - `llmPrompt.ts` —— **Anthropic + OpenAI + Gemini 三 provider**（Phase 2.1）。Gemini 含 `grounding` (Google Search 工具) + `responseFormat: "json"` + `imagePathsField` (vision，all 3 providers 共用)。grounding 与 JSON-mode 冲突时 grounding 赢，自动 fallback 到 text mode；citations 透传到 `_citations`
  - `mcpServer.ts` —— 占位
  - `internal/index.ts` —— slug 映射。当前**只有 3 个**（Phase 5+6 大量删除）：`relic-files-summary` / `relic-smart-image-pick` / `image-to-data-uri`。删的：`meshy-3d` / `relic-cutout` / `relic-image-pick` (Phase 5 R1)、`relic-gemini-researcher` (Phase 6.1) — 全部 forge 化到 LLM_PROMPT/HTTP_API skills
...
- `lib/skills/runtime/{backbone,orchestrator,runner}.ts` —— 编排器 + async runner（含数据驱动 writeback hook）
...
- **v2**（DAG）：`{ version: 2, nodes: [...], edges: [{ from, to, when? }] }`，节点三种 type：
  - `skill`：`{ id, type: "skill", equipSlot, inputFrom, position? }`
  - `branch`：`{ id, type: "branch", inputFrom, cases: [...], defaultLabel?, position? }`
  - `loop`（Phase 8 R1）：`{ id, type: "loop", inputFrom, maxIterations: 1-10, exitWhen?: BranchCase[], body: { nodes, edges } 自包含 sub-DAG, aggregate?: "last"|"concat-array", position? }`
...
**Backbone 编辑器是 React Flow** —— [`BackboneFlowEditor.tsx`](app/agent-control/components/BackboneFlowEditor.tsx) 替换了原来的 JSON textarea。两种节点（SkillNode 金 / BranchNode 珊瑚红）+ LabeledEdge；位置写到 `pipelineConfig.nodes[].position`，下次进来同布局。Test Run 在侧栏，dry-run 整个 DAG。**Loop 节点 UI 待加**（Phase 8 R2 候选）—— 当前 admin 通过 raw JSON / Advanced 模式建 loop。
...
**SkillEditor "What this skill does" preset (Phase 3)** —— 替换原 raw `Handler Type` 下拉。8 个预设（LLM × 3 provider / HTTP × 3 形态 / Internal / MCP），每个含 `defaultConfig` scaffold。`derivePresetKey()` 启发式从现有 skill 反推 preset（HTTP_API 含 `polling` → "async" preset，URL 含 `/api/internal/save-asset` → "save-asset" preset）。raw `handlerKind` 仍能在 Advanced 模式编辑。
...
# post-push: 列存在后做数据 backfill / 创建 forge 配置
  ...
  && tsx prisma/migrate-scene-bindings.ts          # Phase 0a-0b: SCRIBE capabilities + 4 个 default binding
  && tsx prisma/migrate-meshy-forge.ts             # Phase 2.4.1: 3 skill + MESHY-FORGE-001 + rebind create3d
  && tsx prisma/migrate-cutout-forge.ts            # Phase 2.4.2: 2 skill + CUTOUT-FORGE-001 + rebind enhance2d
  && tsx prisma/migrate-phase5-cleanup.ts          # Phase 5 R1: SceneBinding outputMap + 删 SCRIBE 旧 DAG/equip + 删 3 个 obsolete Skill
  && tsx prisma/migrate-lore-forge.ts              # Phase 5 R2: 3 LLM_PROMPT skill + LORE-FORGE-001 + rebind draft/regen-metadata
  && tsx prisma/migrate-phase6-cleanup.ts          # Phase 6.1: 清 SCRIBE pipelineConfig + equips + 删 relic-gemini-researcher Skill
  && next start
```
