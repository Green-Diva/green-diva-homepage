...
- `Session` —— 登录会话。
- `Relic` —— 收藏品。状态机见"Relic 上传流水线 + 状态机"段。**关键列**：`primaryImagePath`（admin 选定的主图）、`enhancedImages` Json[]（fal.ai 抠图结果数组，max 16；shape `{ path, sourceCandidatePath, model, resolution, refineForeground, createdAt }`；**2026-05-18 由 `enhancedImagePath: String?` 改为此数组**，迁移脚本 `migrate-replace-enhanced-image-path.ts`，按 `sourceCandidatePath` upsert；`enhancedImages[0].path` 相当于旧的单值）、`modelPath`（Meshy 输出 GLB，也可由 `POST /api/relics/[id]/model/upload` 直传跳过 Meshy）、`candidateImages` Json（draft pipeline 产出的全候选集 + admin 的 deleted/primary 状态）、`pipelineTrace` Json（最近一次 agent 调用的 runLog）、`loreZh/loreEn`（Markdown）。`photoPaths` 是 admin 手工 upload 用的老字段。
...
app/
  ...
  api/
    ...
    relics/             — GET / POST 列表 + 创建
                          ...
                          [id]/enhance-2d POST 触发 cutout（异步 AgentJob；每张候选独立跑，结果 upsert 到 enhancedImages 数组）
                          [id]/enhanced-item GET?path=<> 查单条 enhancedImage / DELETE?path=<> 删单条（RelicForm 候选删时级联调）
                          [id]/create-3d POST 触发 Meshy（异步；要求 enhancedImages.length > 0）
                          [id]/model/upload POST 直传 GLB（admin-only，multipart，50MB；magic byte 0x676c5446 校验；persist 写盘，写 modelPath，跳过 Meshy 流程）
...
| tab | 状态 | 触发后 |
|---|---|---|
| `original` | 永远可点；candidateImages 分 user / network 两组，各组独立翻页（per-group carousel），来源标签显示；最多 16 张 | — |
| `enhance2d` | admin 通过 [`Cutout2dConfigModal`](app/admin/relics/Cutout2dConfigModal.tsx) 触发（3 步：选图 / BiRefNet 配置 / 结果）→ POST `/enhance-2d`（薄壳 → `dispatchScene("relic.enhance2d", ...)`）| AgentJob 异步跑 fal cutout + `persist` 原语；writeback hook 按 `sourceCandidatePath` upsert 到 `Relic.enhancedImages[]`；运行中显示 `x/y` 进度，界面高度锁定不跳变；取消用 in-modal 确认覆层（不用 `window.confirm`）；删候选图时 RelicForm `pruneEnhancedAgainstCandidates()` 级联删 enhancedImages 中 sourceCandidatePath 匹配项 |
| `model3d` | 依赖 `enhancedImages.length > 0`（**不再是 enhancedImagePath**），没有则 disable；admin 通过 [`Meshy3dConfigModal`](app/admin/relics/Meshy3dConfigModal.tsx) 触发（3 步：选增强图源 / Meshy 参数 + 已有模型状态 + GLB 直传 / 结果预览）→ POST `/create-3d` 或 POST `/model/upload` | `/create-3d`: dispatchScene → RELIC-FORGE-001 (mode=3dCreate)；AgentJob 异步跑 Meshy submit + poll + download + `persist` 原语，写 `Relic.modelPath`。`/model/upload`: 直传 GLB 跳过 Meshy，写 `modelPath`。步骤3 内嵌 RelicViewer 预览（dynamic import 避免加载 @google/model-viewer 进 admin tree）+ GLB 二进制头解析模型属性（magic bytes 0x676c5446 + JSON chunk，显示材质数/节点数等）|

**Cutout2dConfigModal 可配置参数（fal.ai BiRefNet）**：
- `model`：`General Use (Light)` / `General Use (Heavy)` / `Matting`（质量 vs 成本）
- `operating_resolution`：`1024x1024`（默认）/ `2048x2048` / `512x512`
- `refine_foreground`：boolean（精炼前景细节的低成本二次 pass）

**Meshy3dConfigModal 关键 UX**：
- `regenOverride` 三态状态机：无模型 → 直接生成；有模型 + 未勾选 → 设置区灰化（`<fieldset disabled>` 级联）；有模型 + 勾选 → 解锁设置，GLB 直传 tab 同时灰化（互斥）
- step 2 左侧状态 tile 的 icon 与下方 toggle 的 `p-1.5` 对齐（`pl-1.5`）
...
7. **不要让 skill 知道业务概念**。Skill = 原子 IO（一次 HTTP / 一次 LLM），responseTransform 只产 raw 响应字段；**`_relicWriteback` / scene-shape 字段都属于 agent 的封装层**（末尾 `transform` 节点用 JSONata 组装）。skill 不应该出现 `enhancedImages` / `modelPath` 这类业务字段名——那样 skill 就锁死在某个 scene 上，复用性归零。详见"Skill / Agent 责任边界"段。
...
**SkillEditor 表单契约（2026-05-18）** —— [SkillEditor.tsx](app/agent-control/components/SkillEditor.tsx) 已对齐 handler 后端能力，**Advanced (raw JSON) 仅作 escape hatch**：

- HTTP_API 表单覆盖：`method` / `url` / `authEnv` / `authScheme` (6 枚举) + 条件 `authHeader`/`authQueryParam` / `responseType` (json/text/binary) + 条件 `binaryMaxBytes` / `timeoutMs` / `headers` / `queryTemplate` / `bodyTemplate` / `responseTransform` / Async Polling 折叠区（url / method / interval / timeout / headers / successWhen / failureWhen / progressPath / progressLabelPath）/ Async Download 折叠区（urlPath / field / maxBytes）
- LLM_PROMPT 表单覆盖：`provider` / `model` / `systemPrompt` / `userTemplate` / `maxTokens` / `temperature` / `authEnv` / `responseFormat` (text/json segmented) / `imagePathsField` (Vision) / `grounding` (Gemini 限定)
- **mutex / 冗余隐藏（UI 反映 backend 真实行为，改时不要复活）**：
  1. `method === "GET"/"HEAD"` → 藏 Body Template（[httpApi.ts:514](lib/skills/handlers/httpApi.ts) `hasBody` skip）
  2. `responseType === "binary"` → 藏整个 Async Polling 折叠区（backend 互斥 throw `INVALID_CONFIG`）
  3. `provider === "gemini" && grounding === true` → 禁用 `responseFormat=json` 按钮 + 自动清除 stale `responseFormat=json`（backend silently 降级到 text）
  4. `provider === "openai"` → 整段隐藏 Vision `imagePathsField`（OpenAI vision 暂未实现，不显示"看得见但点不了"灰态）
- **两页 Tab 布局（固定 760×768px 外壳，不随内容跳）**：Page 1 = 基础信息（Level/Status/Slug/Icon/Name×2/Description×2/IO Contracts 并排两栏/Equipped By），Page 2 = 运行时（Preset + StructuredHandlerFields 或 Advanced raw JSON）。校验失败自动跳页：schema JSON 错跳 Page 1，handler JSON 错跳 Page 2。body 内部 `overflow-y-auto` 作 safety net。
- **失败模式（form 仅支持单条件，多条件走 Advanced）**：`polling.failureWhen` 数组形式仅在 Advanced JSON 可编；form 模式只渲染"Currently set to an array — edit via Advanced"提示。
- **设计哲学（不要往 schema-driven 通用渲染迁移）**：handler kind 是**有意收口的封闭集合**（呼应"永远不要做 ZIP 上传插件"约束），不是 SaaS-style 开放扩展。结构化 TSX 硬编码换来精致 UX + 类型安全，是符合设计意图的局部最优解。**迁移触发条件**：handler kind 数 > 5、同 kind 内 provider 数 > 5、出现 OAuth / 文件上传 / 多步骤鉴权需求时再讨论 RJSF / formily 路径，目前不要预设计。
...
tsx prisma/migrate-token-hash.ts
  ...
  && tsx prisma/migrate-replace-enhanced-image-path.ts  # Relic.enhancedImagePath String? → enhancedImages Json[]（2026-05-18；backfill 旧单值到数组 + DROP legacy column）
...
# post-push: 列存在后做数据 backfill / 创建 forge 配置
  ...
  && tsx prisma/migrate-lore-prompt-v2.ts          # 强刷 gemini-lore-en/zh 的 systemPrompt 到最新常量（2026-05-18；解决 lore part 3 liturgical-cliché 问题；只覆盖 systemPrompt 字段，其余 handlerConfig 保留）
  && next start
```
