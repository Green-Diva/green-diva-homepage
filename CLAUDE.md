...
- `SceneBinding` —— 把 code-registered scene（`relic.enhance2d` 等）映射到 agent。字段：`sceneKey @unique` / `agentId` / `inputMap Json` (template `{{ctx.X}}` / `{{actor.X}}` 派 ctx → AgentJob.input) / `outputMap Json?` (callScene/runner 用，scope `{ output, runLog: { entries, byId }, ctx, actor }`) / `enabled Boolean` / `notes String?`。`onDelete: Restrict` on agent FK — 删 agent 前必须先 unbind。详见"Agent Service + Forge Agents"段。
...
浏览器 → POST /api/relics/[id]/enhance-2d              ← endpoint 永不变
        ...
        Scene Registry (lib/scenes-init.ts → lib/relics/scenes.ts)：找定义 + 校验 contextSchema
              ↓
        SceneBinding 表 (DB)：admin 改 agentId / inputMap / outputMap
              ↓
...
| Forge | 职责 | 槽位结构 |
|---|---|---|
| **LORE-FORGE-001** | 草稿元数据 + regen（绑 `relic.draft-metadata` + `relic.regen-metadata` scenes） | mode-branch → loreEn (LLM_PROMPT Gemini grounding+vision) → loreZh (LLM_PROMPT Gemini text) → metadata-init (LLM_PROMPT Gemini json+vision)；mode-router 走 metadata-regen 给 regen 模式。**`agent.input.{userBrief,fileSummary,imageAbsPaths,textExcerpts}` 由 pipeline 层 [`scanWorkspace`](lib/relics/pipeline/scanWorkspace.ts) 在 callScene 前预填**。 |
...
| **CUTOUT-FORGE-001** | 2D 抠图（绑 `relic.enhance2d` scene） | fal-cutout-http (HTTP_API + download) → save-asset-enhanced (HTTP_API → /api/internal/save-asset，emit `_relicWriteback`)。**`agent.input.imageDataUri` 由 endpoint 用 [`readRelicImageAsDataUri`](lib/relics/readImageAsDataUri.ts) 在 dispatchScene 前预编码**。 |
| **MESHY-FORGE-001** | 3D 生成（绑 `relic.create3d` scene） | meshy-3d-http (HTTP_API + polling + download) → save-asset-relic (HTTP_API + emit `_relicWriteback`)。`imageDataUri` 同上由 endpoint 预编码。 |

**IO 放哪里：现状 = pipeline/endpoint 层做完，agent 接 ready-to-use 输入**

所有 forge 走"pipeline-准备"路径，agent DAG 不再有 INTERNAL handler。LORE-FORGE 的 `scanWorkspace`、CUTOUT/MESHY 的 `readRelicImageAsDataUri`、PICKER 的 `stageUserCandidates` 都是这个 pattern：把 FS / Prisma 接触面留在 pipeline / endpoint 层，agent 只做 HTTP+LLM 编排。
...
### Pipeline-step / endpoint 解耦

pipeline step 不按硬编码 node ID 从 runLog 抽 output（agent 重命名 node 会破），而是读 `result.output`，由 SceneBinding.outputMap 用 `{{runLog.byId.<nodeId>.output.X}}` 模板塑形。

`callScene` 给 outputMap scope 暴露 `{ output, runLog: { entries, byId }, ctx, actor }`。template engine path 支持 dash 让 `metadata-regen` 这种 node ID 能引用。

### 异步 writeback hook（数据驱动）

[`runner.ts::maybeWriteRelicAsset`](lib/skills/runtime/runner.ts) 单一数据驱动路径：agent leaf output 含 `_relicWriteback: { id, fields }` → 按 **15 字段 allowlist**（`enhancedImagePath` / `modelPath` / `loreZh` 等）写 Relic 列。新增 mode 不动 runner 代码 — 由 SceneBinding outputMap 或 skill responseTransform 产出 _relicWriteback。**不要**依赖 `input.mode` 字段做回写路由。
...
5. **Runner 的 `maybeWriteRelicAsset` writeback hook 是 relic-bound 异步调用的唯一回写路径**（[`lib/skills/runtime/runner.ts`](lib/skills/runtime/runner.ts)）。Agent leaf output 必须含 `_relicWriteback: { id, fields }`（由 SceneBinding outputMap 或 skill responseTransform 产出），fields 走 `ALLOWED_WRITEBACK_FIELDS` 白名单。新增异步回写目标 = 加白名单字段 + 让 binding/skill 产出对应 `_relicWriteback`，不动 runner 代码。**不要**依赖 `input.mode` 字段做回写路由。
...
# post-push: 列存在后做数据 backfill / 创建 forge 配置
  ...
  && tsx prisma/migrate-phase5-cleanup.ts          # SceneBinding outputMap + 删 SCRIBE 旧 DAG/equip
...
下面只列**与项目业务强绑定、读架构必须知道**的几条（其余在 gotchas.md）：

- **i18n 边界**：Server Component 用 `lib/i18n/server.ts`，Client Component 用 `lib/i18n/client.tsx`，两者不可混用。新增字典 key 必须**同时**改 `lib/i18n/types.ts` + `en.ts` + `zh.ts`。
