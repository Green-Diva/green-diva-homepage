...
- `Skill` —— 可装备的能力定义（kind = PASSIVE/ACTIVE/ULTIMATE，level 1-6，icon 走 Material Symbols 名）。CRUD 在 `/agent-control?tab=skills`。**`kind` 当前是纯装饰字段**（仅 [`SkillLibrary.tsx`](app/agent-control/components/SkillLibrary.tsx) 用于徽章配色，零 runtime 语义）；未来 `handlerKind`（INTERNAL / HTTP / LLM_PROMPT 等真正描述"怎么调用"的字段）落地后，`kind` 可能整体废弃。新加任何依赖 `kind` 的逻辑前先想想是否在替这个字段续命。
